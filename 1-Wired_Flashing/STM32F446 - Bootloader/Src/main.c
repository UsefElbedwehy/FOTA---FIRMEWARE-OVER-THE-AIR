...
void Bootloader_UartReadData(void)
{
	...
	while(1)
	{
		...
		/*1st step: Read 1st byte which include the "Length to follow" field of the command*/
		HAL_UART_Receive(&huart1, Local_u8CmdPacket, 1, HAL_MAX_DELAY);
		//HAL_UART_Receive(&huart1, Local_u8CmdPacket, 1, HAL_MAX_DELAY);
		...
		case BL_DIS_WR_PROTECT		:	BL_VoidHandleDisWRProtectCmd(Local_u8CmdPacket); 		break;
...
{
	/*Pointer to function to hold the address of the user application*/
	void (*App_ResetHandler)(void);

	uint32_t ResetHandlerAddress;

	/*Configure the MSP of the user application by reading value from base address of sector2*/
	...
	/*Get reset handler address of the user application*/
